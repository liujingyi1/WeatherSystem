...
public class UpdateWidgetService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (AppWidgetManager.INVALID_APPWIDGET_ID == widgetId) {
			if (WeatherWidget.ACTION_INIT.equals(intentAction)) {
				...
				WeatherApp.mModel.refreshAllWeather();
				...
				Log.v(TAG, "ACTION_WEATHER_REFRESH_ALL ?");
				WeatherApp.mModel.refreshAllWeather();
	...
	private void updateWidgetUI() {
		...
		views.setTextViewText(R.id.widget_weathertemperature,
				(isTemperatureC?mWidgetWeatherInfo.getForecasts().get(0).getLow()
						:mWidgetWeatherInfo.getForecasts().get(0).getLowF())
				+ temperatureType
				+ "/"
				+ (isTemperatureC?mWidgetWeatherInfo.getForecasts().get(0).getHigh()
						:mWidgetWeatherInfo.getForecasts().get(0).getHighF())
				+ temperatureType);
		...
		resId = WeatherDataUtil.getInstance().getWeatherImageResourceByCode(
				code, isnight, WeatherDataUtil.IS_WIDGET);
		...
		if (WeatherDataUtil.INVALID_WEAHTER_RESOURCE == resId) {
			resId = WeatherDataUtil.getInstance()
					.getWeatherImageResourceByText(
							...
							isnight, WeatherDataUtil.IS_WIDGET);
	...
	private void setDefaultInfo() {
		...
		resId = WeatherDataUtil.getInstance().getWeatherImageResourceByText(
				defaultData, isnight, WeatherDataUtil.IS_WIDGET);
